...
- Management History (`api.shieldlabs.ai/v1/history`): **Shield.Core**, PascalCase array — **deprecated in this spec** (`searchHistoryV1`), **Sunset 2027-01-01**. Successor: History API `https://account.shieldlabs.ai/api/v1/history`. Handler still live and emits `Deprecation` / `Sunset` / `Link` headers; do not document per-row billing (code does not debit).
